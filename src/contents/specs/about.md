## Information

<div class="flex flex-col md:flex-row items-center justify-center text-left md:items-center gap-5">
   
  <div class="flex shrink-0">
    <img
      src="https://media.licdn.com/dms/image/v2/D4D03AQHGPFX08deonw/profile-displayphoto-crop_800_800/B4DZjCPmu2GgAI-/0/1755605530493?e=1758758400&v=beta&t=wJm__8mVYKLTRFNVpbENWDwJSaVX_G9rhPbDL9BOd1g"
      alt="Profile Picture"
      class="flex w-40 h-40 rounded-xl object-cover shadow"
    />
  </div>
  <div class="">
    <p><strong>Fullname:</strong>  Lê Nhựt Quốc Khang </p>
    <p><strong>DoB:</strong> 28/03/2005 </p>
    <p><strong>Email:</strong> lnqkhang.insec@gmail.com </p>
    <p><strong>Discord:</strong> raviyelna </p>
    <p><strong>Telegram:</strong> Ravi_LNQK </p>
  </div>
  <div class="flex items-center justify-center">
      <img
      src="https://lanyard.cnrad.dev/api/415787209661087744?animated=true&animatedDecoration=true&hideDecoration=false"
      alt="Discord profile"
      class="flex object-cover shadow"
    />
</div>

</div>






## Technical stuffs
- Programming: Python, C++, C# (vibe coding 60% of the time)
- Tools: IDA Pro, x64dbg, WinDbg, Splunk, Wireshark, Burp Suite, Volatility, FTK imager, Autopsy,...
- Operating Systems: Windows, Linux
- Languages: English, Vietnamese
- Interests: 
  * Malware Analysis
  * Reverse Engineering
  * Digital Forensics Incident Response
  * Malware Researching
  * Cybersecurity
  * CTF Competitions
- Current Status: 
  * 3rd years student at HCMUTE
  * Cloud Security at FPT software Cybersecurity Assurance Service
  * BlitzHack CTF Team Moderator


## Achievements

- 1st Prize HCMUTE CTF 2024
- Consolation Prize at ASEAN STUDENT CONTEST ON INFORMATION SECURITY 2024




<div class="flex flex-col md:flex-row items-center justify-center text-left md:items-center gap-20">
      <img
      src="https://pbs.twimg.com/media/GyxWQP1acAQMhXY?format=jpg&name=4096x4096"
      alt="Picture"
      class="flex rounded-xl object-cover shadow"
    />

  </div>



  